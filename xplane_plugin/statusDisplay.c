...
void keyStatus(XPLMWindowID inWindowID, char inKey, XPLMKeyFlags inFlags,
...
void statusDisplayInit(void) {
  memset(textStatus, 0, sizeof(textStatus));
	...
	if (statusDisplayShow) {
		if (!wStatus) {
			wStatus = XPLMCreateWindow(50, 590, 950, 50, 1,
				drawStatus, keyStatus, mouseStatus, NULL);
			headStatus = 0;
...
int statusPrintf(const char *format, ...) {
	...
	if (++headStatus >= LINES) headStatus = 0;
