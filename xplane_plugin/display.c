...
void key(XPLMWindowID inWindowID, char inKey, XPLMKeyFlags inFlags, char inVirtualKey, void *inRefcon, int losingFocus) {
}
...
void TeensyControls_display_init(void) {
   ...
   if (TeensyControls_show) {
      if (!w) {
         w = XPLMCreateWindow(50, 590, 950, 50, 1, draw, key, mouse, NULL);
         head = 0;
