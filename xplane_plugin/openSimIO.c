...
void createWindow() {
        ...
        XPLMSetWindowResizingLimits(g_window, 400, 400, 3000, 3000);
...
PLUGIN_API int XPluginStart(char* outName, char* outSig, char* outDesc) {
    ...
    myMenu = XPLMCreateMenu("openSimIO",
                            XPLMFindPluginsMenu(),
    ...
    XPLMAppendMenuItem(myMenu, "Calibration window", (void*)6, 1);
...
void draw_hello_world(XPLMWindowID in_window_id, void* in_refcon) {
    ...
    int char_height;
    XPLMGetFontDimensions(xplmFont_Basic, NULL, &char_height, NULL);
    char_height = 24;
    float col_white[] = {1.0, 1.0, 1.0}; // red, green, blue

    // XPLMDrawString(col_white, l + 10, t - 20, "Hello world!", NULL, xplmFont_Proportional);
    // XPLMDrawString(col_white, l + 20, t - 20, "Hello world2!", NULL, xplmFont_Proportional);
    char lineout[1024];
    glEnable(GL_LINE_SMOOTH);
    glEnable(GL_POLYGON_SMOOTH);
    for (int i = 0; i < nrOfPins; i++) {
        if (pins[i].output == 1) {
            snprintf(lineout, 1024,"%s = %d <- %s=%f \n", pins[i].pinNameString, pins[i].prevValue, pins[i].dataRefString, pins[i].lastSimValue);
        } else {
            snprintf(lineout,1024, "%s = %d -> %s=%f \n", pins[i].pinNameString, pins[i].prevValue, pins[i].dataRefString, pins[i].lastSimValue);
        }
        // XPLMDrawString(col_white, l , t  -char_height*i, lineout, NULL, xplmFont_Basic);
        drawLineText(lineout, l , t  -char_height*i, char_height*0.04,0);
    }
}
        ...
        if (masters[i].type == IS_SERIAL) {
            ...
            if (n > 0) {
                ...
                if (ifCharInArray(buf, '}') == -1) {
                    ...
                    display("    received broken message %i bytes: %s\n", n, (char*)buf);
                } else {
